...
- **[`app/CLAUDE.md`](./app/CLAUDE.md)** — the FastAPI service's build context (Dockerfile, Makefile, requirements, the entrypoint shim, why `privatize_this_config.py` sits one level up).
- **[`app/ppi/CLAUDE.md`](./app/ppi/CLAUDE.md)** — package internals: the agent / gliner_provider / anonymize call path, contextvar plumbing, model dispatch, editing checklist.
...
```
app/                       FastAPI service — self-contained build context (see app/CLAUDE.md)
...
Makefile                   Compose orchestration (dc-up/down/restart, ollama-pull-models). Image build lives in app/Makefile.
...
The root `Makefile` is compose-only. Image build and Python env live in `app/` — see [`app/CLAUDE.md`](./app/CLAUDE.md).

```bash
# Dev stack (all active includes — see compose/) + pull Ollama PII models
make                                # = make dc-up; docker compose up -d
make ollama-pull-models             # pulls the GGUF PII LLMs listed in Makefile MODELS
...
# Build / run the FastAPI image — see app/CLAUDE.md (cd app && make)
```
...
- **`PII_LABELS`** (in `app/privatize_this_config.py`) drives both backends: GLiNER's zero-shot detection (via `labels_ctx`) and Ollama's system prompt. Adding a label here changes both at once.
- **Stateless FastAPI image** — see `app/CLAUDE.md` for build mechanics. The image bakes all four GLiNER models and runs `HF_HUB_OFFLINE=1` so `readOnlyRootFilesystem: true` is viable in Kubernetes.
...
- **Ollama-only `MODELS` list in the root `Makefile`** — these are GGUF PII LLMs pulled into the Ollama container's volume. The four GLiNER HF model IDs are NOT pulled here (they aren't GGUF); they're baked into the FastAPI image at build time (and listed in both `app/Dockerfile` and `app/privatize_this_config.py:ModelName` — change one, change the other).

## Gotchas

- **For package internals** (call path, contextvar plumbing, `ollama/`→`ollama_chat/` rewrite, `PromptedOutput` requirement, overlap resolution, etc.), see `app/ppi/CLAUDE.md`. **For build / Python env / Dockerfile**, see `app/CLAUDE.md`. This file deliberately does not duplicate either.
- **`pydantic-ai-slim[litellm]` import paths vary by version.** `app/ppi/agent.py` tries `pydantic_ai.models.litellm.LiteLLMModel` first, then falls back to `pydantic_ai.models.openai.OpenAIModel` with a `LiteLLMProvider`. Pin or upgrade pydantic-ai rather than working around its current shape.
